...
async function main() {
    ...
    if (sourceData.dictionary && sourceData.dictionary['zh-TW']) {
        ...
        console.log("ℹ️ Detected manual-dictionary structure, using 'zh-TW' subtree.");
    }
    ...
    try {
        ...
        if (targetData.dictionary && targetData.dictionary.en) {
             console.log("ℹ️ Detected output file has dictionary structure, merging into 'en' subtree.");
    ...
    for (let i = 0; i < chunks.length; i++) {
        ...
        console.log(`🤖 Translating batch ${i + 1}/${chunks.length} (${Object.keys(chunk).length} keys)...`);
    ...
    console.log(`✅ Translation complete! ${processedCount}/${missingKeys.length} keys translated.`);
